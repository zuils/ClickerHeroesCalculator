...
function hpScaleFactor() {
    ...
    var scale;
    if (zone < 141) {
        scale = new Decimal(1.55);
    } else if (zone < 501) {
        scale = new Decimal(1.145);
    } else if (zone < 200001) {
        scale = i.times(0.001).plus(1.145);
    } else {
        scale = new Decimal(1.545);
    }
    return scale;
}

function alphaFactor(heroTypeSelected) {
    if (heroTypeSelected == "e10") {
        return data.tp.dividedBy(100).plus(1).ln().times(1.8053).dividedBy(hpScaleFactor().ln());
...
function computeOptimalLevels(tuneAncient, addLevels) {
    var alpha = alphaFactor(data.settings.heroTypeSelected);
    ...
    for (var k in data.ancients) {
        ...
        if (oldLevel.greaterThan(0) || k == "soulbank") {
            ...
            if (goalFun) {
                var g = goalFun(baseLevel, oldLevel, alpha, transcendent, data.settings.heroTypeSelected, hybridRatio);
